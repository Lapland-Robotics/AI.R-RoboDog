...
float _targetPos[12] = {0.0, 1.4, -2.67, 
                        0.0, 1.4, -2.67, 
                        0.0, 1.4, -2.67, 
                        0.0, 1.4, -2.67};
float _startPos[12];
float rate = 50;
float _duration = rate;   //steps
float _percent = 0;       //%
...
void lowStateCallback(const unitree_legged_msgs::LowState::ConstPtr &msg)
{
    ...
    if (!posSet){
        for (int i = 0; i < 12; i++)
        {
            ...
            lowCmd.motorCmd[i].q = _startPos[i];
        }
...
void init(){
    ...
    for (int leg = 0; leg < 4; leg++){
        ...
        lowCmd.motorCmd[leg * 3 + 1].Kp = 40;
        lowCmd.motorCmd[leg * 3 + 1].Kd = 4;
...
{
    ros::init(argc, argv, "example_crouching_without_lcm");
    ...
    while (ros::ok())
    {
        ...
        for(int j=0; j<12; j++){
            lowCmd.motorCmd[j].q = (1 - _percent) * _startPos[j] + _percent * _targetPos[j];
        }
