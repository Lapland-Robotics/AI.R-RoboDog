...
Vec12 targetPos = (Vec12() << 0.0, 0.8, -1.6, 0.0, 0.8, -1.6, 0.0, 0.8, -1.6, 0.0, 0.8, -1.6).finished();
Waypoint targetWaypoint = Waypoint(targetPos, 2.0f); // 2 seconds to reach target
Vec12 startPos;
float rate = 50; // Hz
...
bool standUpCallback(__attribute__((unused)) std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
{
    if (motionInProgress) {
        res.success = false;
        res.message = "Stand up motion already in progress.";
        return true;
    }
    
    ROS_INFO("Service stand_up triggered.");
    ...
    res.success = true;
    res.message = "Robot standing up completed.";
    return true;
}

int main(int argc, char **argv)
{
    ros::init(argc, argv, "stand_up_service");
    ros::NodeHandle nh;
    ...
    ros::ServiceServer service = nh.advertiseService("/stand_up_srv", standUpCallback);

    ROS_INFO("Service stand_up ready. Waiting for calls...");
    ros::spin();
    return 0;
}
