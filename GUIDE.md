...
We also provide the `state_pub` node that publish the state of the robot without sending the commands. It is mainly used to save the state of the robot when performing highlevel actions and try to reproduce them.

## Commands the robot

There are a few nodes used to publish the lowlevel commands to the UnitreeGo1. The nodes `stand_up_srv`, `crouch_srv`, `sit_srv` and `give_paw_srv` are services nodes triggered by the `cmd_dispatcher` node. The nodes `stand_up`, `crouch` and `sit` does the same as their services homologues but are triggered using ROS CLI (e.g. `rosrun robotdog stand_up`) and doesn't need the commands nodes to be running. To send the commands to the robot, you always require the `udp_ros` to be running.
...
This project contains one launch file that allows us to launch commands interfaces, udp interface in lowlevel mode and all services at the same time to control the robot using the keyboard.